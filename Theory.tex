...
$$p(Y=1|X=x)=\frac{f_1(x)q_1}{f_1(x)q_1+f_0(x)q_0},$$
$$p(Y=0|X=x)=\frac{f_0(x)q_0}{f_1(x)q_1+f_0(x)q_0}.$$
...
The risk for a general $K$ would be $R^{K-NN}=P(Y\neq \text{majority}\{Y_{(1)},\dots,Y_{(K)}\})$. For $K=3$, $R^{3-NN}=P(Y\neq \text{majority}\{Y_{(1)},\dots,Y_{(K)}\})=\mathbb{E}[\eta(x)(1-\eta(x))]+4\mathbb{E}[\eta(x)^2(1-\eta(x))^2]\leq2\mathbb{E}[\eta(x)(1-\eta(x))]=R^{NN}$. If we develop in general for any $K$, we get
\begin{align}
	R^{K-NN} = &\mathbb{E}[\eta(X)\mathbb{P}(\text{Bin}(K,\eta(X))<\frac{K}{2}|X)+(1-\eta)\mathbb{P}(\text{Bin}(K,\eta(X))>\frac{K}{2}|X)]=\\
	= & \mathbb{E}[\min(\eta(X),1-\eta(X))]+\mathbb{E}[(2\eta(X)-1)\mathbb{P}(\text{Bin}(K,\min(\eta,\eta-1))>\frac{K}{2}|X)]\leq\\
