...
\vfill
\begin{center}
	\small{Source: Lectures of Gabor Lugosi (Barcelona Graduate School of Economics - MSc.Data Science 2016-2017)}
\end{center}
\begin{center}
	\small{Acknowledgments: Akhil Lohia, Laura Roman, Alfonso Braulio, Veronika Kyuchukova}
\end{center}
\newpage
\tableofcontents
\thispagestyle{empty}
...
Ifa $X\geq 0$ is a non-negative random variable and $t>0$, then
\begin{equation}
...
1. We start with an arbitrary $w_0$, for example $w_0=\overset{\rightarrow}{0}$.\\
...
Suppose that $X_i \in R^d$. We may define a ``feature map'' $\Phi : R^d \rightarrow R^D$ where $ D>d$, work with $(\Phi(X_i), Y_i), \dots, (\Phi(X_n), Y_n)$ and do linear classification in $R^D$. If one achieves a classifier that correctly classifies most of the data with a high margin, then the generalization error is small. The key of kernel methods methods is the inner product $<\Phi(x), \Phi(y)> = \Phi(x)^T \Phi(y) = K(x,y)$.\\%Roger

\textbf{Def.} Let $\Omega$ be a vector space (if $x, y \in \Omega$ then $ax + by \in \Omega \ \forall \ a,b \in \mathbb{R})$. An inner product assigns a real number to any pair $x,y \in \Omega$. An inner product is an operator $<\cdot,\cdot>:\Omega\times\Omega\rightarrow\Re$ such that %Roger
\begin{align}
	&<x,y> = <y,x>\\
	&<ax + by, z> = a<x,z> + b<y, z>\\
...
A linear function in the linear space generated by $\Phi$ is: %Roger
$$f(x_1,x_2) = ax^2_1 + bx^2_2 + c\sqrt{2}x_1x_2, \text{ which is a quadratic function in}\ \mathbb{R}^2$$ %Roger
...
Hence, in $\mathbb{R}^2$, $K(x,y) = (x^Ty)^2$ is a kernel function that induces an inner product in $\mathbb{R}^3$.\\
